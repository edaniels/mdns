...
func Server(multicastPktConnV4 *ipv4.PacketConn, config *Config) (*Conn, error) { //nolint:gocognit
	...
	if multicastPktConnV4 == nil && config.MulticastConnV6 == nil {
		return nil, errNoPacketConn
	}
	multicastPktConnV6 := config.MulticastConnV6
				...
				for _, localName := range c.localNames {
					if localName == q.Name.String() {
						...
						if queryWantsV4 {
							...
							if !isSupportedIPv6(localAddress) {
